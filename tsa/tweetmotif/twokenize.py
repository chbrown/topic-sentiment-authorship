...
import re
import sys
from tsa.tweetmotif import emoticons
...
Url = (r'\b' +
       ...
       UrlBody +
       pos_lookahead(('(%s)?' % UrlExtraCrapBeforeEnd) + UrlEnd))
...
Protect_RE = re.compile(regex_or(*ProtectThese), re.UNICODE)
...
def simple_tokenize(text):
    ...
    res = []
    for bad_i, bad in enumerate(bads):
        res += goods[bad_i]
        res.append(bad)
    res += goods[-1]
...
AposS = re.compile(r"(\S+)('s)$", re.UNICODE)
...
EdgePunctLeft_RE = re.compile(EdgePunctLeft, re.UNICODE)
EdgePunctRight_RE = re.compile(EdgePunctRight, re.UNICODE)
